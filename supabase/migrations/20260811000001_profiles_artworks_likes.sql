-- profiles / artworks / artwork_likes
-- greenfield でも既存 main でも通るよう IF NOT EXISTS 中心
...
update public.artworks
set kind = case
  when prompt_id like '%:sheet' or prompt_id like '%:sheet:%' or storage_path like '%/sheet-%'
    then 'sheet'
  else coalesce(kind, 'drawing')
end
where kind is null
   or (kind = 'drawing' and (
...
create index if not exists idx_artworks_kind on public.artworks(kind, created_at desc);
